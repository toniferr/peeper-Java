package com.toniferr.peeperJava.jdk8.streams;
...
public class Pruebas {
	...
	public static void main(String[] args) {
		...
		System.out.println("Recorriendo una coleccion");
		for (int a = 0; a < facturas.size(); a++) {
			// System.out.println(facturas.get(a));
		}

		System.out.println("Java 5: forEach, que utiliza un iterador");
		for (Factura f : facturas) {
			// System.out.println(f);
		}

		System.out.println("Java 8: nuevo metodo en la interfaz collection para recorrer colecciones:");
		//Factura f = new Factura();
		//Consumer<Factura> c = 
		facturas.forEach(uo -> System.out.println("---" + uo));
		...
		System.out.println("===============foreach(void)===============");
		System.out.println("Recibe un consumer");
		facturas.stream().forEach(f -> System.out.println(f));

		System.out.println("===============.filter(boolean)===============");
		System.out.println("Recibe un predicate");
		facturas.stream().filter(f -> f.getTotal() < 200).forEach(f -> System.out.println(f));

		System.out.println("Concatenando filtros. Óptimo poner filtro más restrictivo primero");
		int id = 3;
		facturas.stream().filter(f -> f.getCliente().getId() == id).filter(f -> f.getTotal() < 200)
				.forEach(f -> System.out.println(f));

		System.out.println("Cada objeto que sale del stream recorre toda la cadena antes de que salga el siguiente");
		...
		System.out.println("devuelve un iterador. Lo utilizamos para controlar nosotros el flujo de objetos que salen del stream");
		...
		System.out.println("===============allMatch(boolean)===============");
		System.out.println("combrueba si todos los elementos cumplen un predicado");
		boolean ok1 = facturas.stream().allMatch(fra -> fra.getTotal() > 0);
		System.out.println("Todas las facturas >0:" + ok1);

		System.out.println("===============anyMatch(boolean)===============");
		System.out.println("comprueba si algún elemento cumple un predicado");
		boolean ok2 = facturas.stream().anyMatch(fra -> fra.getTotal() > 223200);
		System.out.println("Alguna de las facturas >223200:" + ok2);


		System.out.println("Máximo/mínimo con un comparador");
		...
		System.out.println("===============Max(Optional<E>)===============");
		...
		System.out.println("Máximo/mínimo con un comparador lambda");
		Comparator<Factura> cf2 = (fra1, fra2) -> fra1.getTotal().intValue() - fra2.getTotal().intValue();
		System.out.println("===============Min(Optional<E>)===============");
		Optional<Factura> fMin = facturas.stream().min(cf2);
		if (fMin.isPresent()) {
			System.out.println("Factura mínima:" + fMin.get());
		}
		...
		System.out.println("Se salta cierto numero de elementos del stream");
		facturas.stream().skip(3).forEach(fra -> System.out.println(fra));


		System.out.println("===============map(T -> U)===============");
		System.out.println("Para obtener un stream de objetos distintos de los originales");
		facturas.stream().map(fra -> fra.getCliente()).distinct().forEach(c -> System.out.println(c));

		System.out.println("lo mismo que arriba");
		...
		System.out.println("Usando map podemos tambien modificar objetos. " +
				"No es obligatorio que map devuelva un stream de tipo distinto");
		...
		System.out.println("===============collect(Collector.toList())===============");
		System.out.println("para agregar el resultado");
		System.out.println("Necesitamos los dtos del ejemplo anterior en una lista");

		System.out.println("Sin colectores");
		List<FacturaDTO> facturasDTO = new ArrayList<>();
		facturas.stream().map(fra -> new FacturaDTO(fra.getCodigo(), fra.getCliente().getNombre(), fra.getTotal()))
				.forEach(fDTO -> facturasDTO.add(fDTO));

		System.out.println("con collectors");
		...
		System.out.println("Otros ejemplos con collectors");
		...
		System.out.println("===============collect(Collector.toMap())===============");
		System.out.println("Obteniendo un map java. El collector a mapa recibe dos funciones.");
		Map<Integer, Factura> mapa = facturas.stream().collect(Collectors.toMap(f -> f.getId(), f -> f));
		//System.out.println("hasta java 7 set<T> = mapa.keyset() se recorre con bucle for");
		//Set<Integer> claves = mapa.keySet();
		//for (Integer k : claves) {
			// System.out.println(mapa.get(k)); }
		//}
		System.out.println("En java 8 tenemos el metodo forEach para recorrer mapas");
		mapa.forEach((k, v) -> System.out.println(k + ":" + v));

		System.out.println("===============collect(Collector.averagingDouble())===============");
		...
		System.out.println("===============collect(Collector.averagingInt())===============");
		Double media2 = numeros.stream().collect(Collectors.averagingInt(num -> num));
		System.out.println("Media de los números:" + media2);

		System.out.println("===============collect(Collector.summarizingInt())===============");
		System.out.println("Cálculo de varias operaciones, media, count, max, min, sum..");
		IntSummaryStatistics movida = numeros.stream().collect(Collectors.summarizingInt(num -> num));
		System.out.println(movida);

		System.out.println("===============collect(Collector.summarizingDouble())===============");
		...
		System.out.println("===============collect(Collector.joining())===============");
		System.out.println("obtenemos un string a partir del stream");
		String txt = numeros.stream().map(num -> num.toString()).collect(Collectors.joining(", ", "Numeros: ", "."));
		System.out.println(txt);

		System.out.println("===============collect(Collector.groupingBy())===============");
		System.out.println("Agrupando facturas por el cliente");
		...
		System.out.println("Agrupando facturas por el id del cliente");
		Map<Integer, List<Factura>> rs2 = facturas.stream().collect(Collectors.groupingBy(fra -> fra.getCliente().getId()));
		...
		System.out.println("Sin flat map");
		...
		System.out.println("Con flat map");
